...
def main(n_images=30, library="gradcam", method="GradCAM", magnitude=0.1):
    """
    ...
    for model_name in MODEL_NAMES:
        ...
        for filename, image in zip(filenames, images):
            # 3-row subplot: row 1 (perturbed image), row 2 (explanations), row 3 (probabilities)
            fig, ax = plt.subplots(nrows=3, ncols=len(PERTURBATIONS), figsize=(12, 8))

            # Suptitle for model name, image file, and magnitude, closer to the first row
            plt.suptitle(
            ...
            # In the last row, set x-limits and ticks to [0, 0.25, 0.5, 0.75, 1.0], but do not enlarge plots
            for col_idx in range(len(PERTURBATIONS)):
                ax[2, col_idx].set_xlim(0, 1)
                ax[2, col_idx].set_xticks([0, 0.25, 0.5, 0.75, 1.0])
            ...
            if hasattr(fig, "supxlabel"):  # Matplotlib 3.4+ feature
                fig.supxlabel("Noise fraction", fontsize=FONT_SIZES["label"], y=0.04)
            else:
                plt.xlabel("Noise fraction", fontsize=FONT_SIZES["label"])

            # Minimize whitespace among subplots:
            # Decrease top so the suptitle is closer; reduce hspace/wspace for tight columns/rows
            fig.subplots_adjust(
                top=0.88,
                bottom=0.06,
                left=0.04,
